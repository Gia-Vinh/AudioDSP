...
def optimalZeropad(x, fs, f):
    ...
    """
    ## Your code here
    #Divide the sampling rate by the angular freq. of the sin wave to get the minimum no. of sample needed 
    minNoSample = int(fs/f)
    M = x.size                                                                      #get the size of input x
    noZeroPad = 0                                                                #No of zero padding initiated to zero
    if ( (M%minNoSample) == 0):          #If the signal size contains exaclty an integer no. of peridods of the sine
        noZeroPad = 0                                 #noZeroPad is 0 as we dont have to apply zero - padding
    else: 
        noZeroPad = minNoSample - (M%minNoSample)    #or else an appriate no of zero pad is needed
    zeroPads = np.zeros(noZeroPad)
    x = np.append(x, zeroPads)      #add zero-pads at the end of the input signal
    X = fft(x)                                                      #perform fft
    N = x.size
