...
def suppressFreqDFTmodel(x, fs, N):
    ...
    Inputs:
        x (numpy array
    Outputs:) = input signal of length M (odd)
    ...
    ## Your code here
    mX, pX = dftAnal(x, w, N)
    y = dftSynth(mX, pX, w.size) * outputScaleFactor                #synthesize the orignial signal first
    
    #Manipulate mX and pX to produce yfilt here
    
    #get copy of mX and pX
    mXfilt = mX.copy()
    pXfilt = pX.copy()
    #the pin no that corresponding to 70Hz
    pinResolution = fs/N
    pin70Hz = int(np.ceil(70/pinResolution))
    #Filtering
    mXfilt[:pin70Hz + 1] = -120
    
    #synthesize yfilt
    yfilt = dftSynth(mXfilt, pXfilt, w.size) * outputScaleFactor    
