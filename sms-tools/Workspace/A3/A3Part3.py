...
def testRealEven(x):
    ...
    """
    ## Your code here
    isRealEven = False                       #initialize isRealEven boolean as False
    M = x.size                                  #get size of x
    dftbuffer = np.zeros(M) 
    dftbuffer[:M/2+1] = x[M/2:]             #perform zero-phase windowing
    dftbuffer[M/2+1:] = x[:M/2]
    
    X = fft(dftbuffer)                          #get fft of dftbuffer
    imX = np.imag(X)
    isRealEven = np.allclose(imX, np.zeros(M), 10*np.exp(-6))
